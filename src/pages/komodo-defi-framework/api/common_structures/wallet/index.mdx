...
Returned from the [`get_private_keys`](/komodo-defi-framework/api/v20/wallet/get_private_keys/) method, when `"mode": "hd"`.

| Structure | Type                | Description                         |
| --------- | ------------------- | ----------------------------------- |
| coin      | string              | the coin ticker                     |
| addresses | array of HdKeysInfo | array of derived addresses and keys |
...
| Structure        | Type              | Description                           |
| ---------------- | ----------------- | ------------------------------------- |
| derivation\_path | string            | BIP44 derivation path used            |
| pubkey           | string            | public key                            |
| address          | string            | derived address                       |
| priv\_key        | string            | private key in appropriate format     |
| viewing\_key     | string (optional) | viewing key for ZHTLC coins like ARRR |
...
Returned from the [`get_private_keys`](/komodo-defi-framework/api/v20/wallet/get_private_keys/) method, when `"mode": "iguana"`.
...
| priv\_key | string | private key in WIF or appropriate format |
...
| balance          | object | A map/object where keys are token/coin tickers and values are [BalanceInfo](/komodo-defi-framework/api/common_structures/wallet/#balance-info) objects. Single-token platforms (like KMD) will have one entry, while multi-token platforms (like ETH) can have multiple entries for different tokens. |
