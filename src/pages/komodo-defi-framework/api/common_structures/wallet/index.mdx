...
| balance          | object | A map/object where keys are token/coin tickers and values are [balanceInfo](/komodo-defi-framework/api/common_structures/#balance-info) objects. Single-token platforms (like KMD) will have one entry, while multi-token platforms (like ETH) can have multiple entries for different tokens. |
